...
def color_space_transformations(image,Hue,RGBs,Brightness,shuffle,Cla):
    transform = A.Compose([
        ...
        A.RandomBrightnessContrast(brightness_limit=0.2, contrast_limit=0.2, p=Brightness),
...
def back_end(path,transformation_functions=[apply_geometric_transform, color_space_transformations, kernel_filters],rotate=0.5,shift=0.5,scale=0.5,elastic=0.5,blur=0.5,meblur=0.5,gaublur=0.5,motion=0.5,emboss=0.5,Hue=0.5,RGBs=0.5,Brightness=0.5,shuffle=0.5,Cla=0.5):
    for i in range(100):
        image = read_img(path)
        selected_functions = random.sample(transformation_functions, random.randint(0,len(transformation_functions)-1)) 
        for function in selected_functions:
            if function == apply_geometric_transform:
                ...
                image = color_space_transformations(image,Hue,RGBs,Brightness,shuffle,Cla)
            else:
